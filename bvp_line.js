...
function updateTempChart(data) {
  ...
  const verticalLine = chartGroup.append("line")
      ...
      .on("mousemove", function(event) {
          ...
          tooltip
              .html(`Seconds Elapsed: ${closestTime}<br>Baseline Temp: ${closestBaseline.bvp.toFixed(2)}<br>Cognitive Load Temp: ${closestCog.bvp.toFixed(2)}`)
              .style("left", `${event.pageX + 10}px`)
              .style("top", `${event.pageY - 28}px`)
              .style("visibility", "visible");
      })
...
async function main() {
    ...
    async function dropdownChanger() {
        ...
        <select id="bvp-grouping-select">
          ...
          document.querySelector("#bvp-grouping-select").addEventListener("change", handleDropdownChange);
