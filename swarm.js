...
function updateChart(data, maxTimeElapsed, selectedColors) {
  ...
  circles = enterCircles.merge(circles).attr("fill", (d) => colorScale(d.temp));
  ...
  let simulation = d3
    ...
    .force(
      "x",
      d3.forceX((d) =>
        xScale(d.gamified_or_no === "gamified" ? "Gamified" : "Non-Gamified")
      )
    ) // Keep points in x categories
    .force(
      "y",
      d3.forceY((d) => yScale(d.eda))
    ) // Keep points at the right height
  ...
  circles
    .on("mouseover", function (event, d) {
      tooltip
        ...
        .html(
          `EDA: ${d.eda.toFixed(2)}<br>Temperature: ${d.temp.toFixed(2)} °C`
        );
    })
